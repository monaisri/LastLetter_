...
let wordCount = 0;
let streak    = 0;
let best      = parseInt(localStorage.getItem('ll_solo_best') || '0');
...
function usePotion(type) {
  // ถ้าสกิลใดสกิลหนึ่งกำลังทำงานอยู่ → ห้ามใช้
  if (timerPaused || slowActive) {
    setStatus('สกิลกำลังทำงานอยู่ รอก่อนนะ!', 'err');
    return;
  }

  // ถ้าไม่มีสกิลนั้นเหลือแล้ว → ใช้ไม่ได้
  if (potions[type] <= 0) {
    setStatus('ไม่มีสกิลนั้นแล้ว!', 'err');
    return;
  }
  ...
  if (type === 'freeze') {
    // --- Freeze: หยุดเวลา 5 วินาที ---
    timerPaused = true;
    setStatus('❄️ Freeze! เวลาหยุดอยู่ 5 วิ', 'ok');
    renderTimer();

    // หลังจาก 5 วินาที → ปิด Freeze
    ...
    // --- Slow: เวลาเดินช้าลงครึ่งนึง 8 วินาที (จริงๆ คือ 16 tick) ---
    slowActive    = true;
    slowTickCount = 0;
    setStatus('Slow! เวลาเดินช้าลงอยู่ 8 วิ', 'ok');

    // หลังจาก 8 วินาที → ปิด Slow
    effectTimeout = setTimeout(function() {
      slowActive    = false;
      slowTickCount = 0;
      setStatus('');
...
function updatePotionUI() {
  ...
  if (btnFreeze) {
    if (!gameActive) {
      // ยังไม่เริ่มเกม → ปิดปุ่ม
      btnFreeze.disabled = true;
    } else if (timerPaused || slowActive) {
      // สกิลอื่นกำลังทำงาน → ปิดปุ่ม
      btnFreeze.disabled = true;
    } else if (potions.freeze <= 0) {
      // ไม่มีสกิลเหลือ → ปิดปุ่ม
      btnFreeze.disabled = true;
    } else {
      // ใช้ได้ → เปิดปุ่ม
...
function submitWord() {
  ...
  if (wordCount > best) {
    best = wordCount;
    localStorage.setItem('ll_solo_best', best);
  }
